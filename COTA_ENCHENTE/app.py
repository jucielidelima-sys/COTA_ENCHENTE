...
with st.sidebar:
    st.markdown("---")
    arquivo_upload = st.file_uploader("Atualizar base Excel", type=["xlsx"], help="Opcional. Use para trocar a base sem alterar o código.")

base_encontrada = arquivo_upload if arquivo_upload is not None else localizar_base_excel()
if base_encontrada is None:
    st.error("Base Excel não encontrada. Confirme se o arquivo está na pasta base/ com o nome COTA_ENCHENTE.xlsx ou envie pelo campo Atualizar base Excel na lateral.")
    st.stop()
