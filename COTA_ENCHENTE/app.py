...
BASE_PATH = Path(__file__).parent / "base" / "COTA_ENCHENTE.xlsx"
...
def calcula_impacto(cota_num: object, acesso: object, nivel: float) -> str:
    """
    Regra operacional:
    - Se o acesso até a empresa estiver marcado como NÃO e a cota for N/P, considera impactado.
    - Se houver cota numérica, considera impactado quando o nível do rio for maior ou igual à cota.
    - Acesso SIM não entra como impactado, mesmo se houver cota informada.
    """
...
def calcula_prioridade(cota_num: object, acesso: object, nivel: float) -> str:
    ...
    P1: impacto imediato na cota atual ou N/P com acesso NÃO.
    P2: atenção alta quando a cota está até 1m acima do nível atual e o acesso é NÃO.
    P3: monitoramento quando a cota está até 2m acima do nível atual e o acesso é NÃO.
    P4: sem ação imediata.
    """
    acesso_nao = normalize_text(acesso) == "NÃO"
    if acesso_nao and pd.isna(cota_num):
        return "P1 - AÇÃO IMEDIATA"
    if acesso_nao and not pd.isna(cota_num):
...
st.download_button(
    "⬇️ Baixar base filtrada em Excel",
    data=to_excel_bytes(filtered[show_cols]),
    file_name="base_filtrada_risco_enchente.xlsx",
    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
)
...
with st.expander("ℹ️ Como o dashboard calcula o risco"):
    st.write(
        "O colaborador é classificado como impactado quando o acesso até a empresa está como NÃO e a cota cheia é menor ou igual ao nível do rio simulado. "
